...
void CMemory::MapRAM ()
{
	...
	REPACK_BIRAM(c + 0x7e0);
	REPACK_BIRAM(c + 0x7f0);
    }
...
void CMemory::MapExtraRAM ()
{
	...
	REPACK_BIRAM(c + 0x700);
	REPACK_BIRAM(c + 0x710);
	REPACK_BIRAM(c + 0x720);
	REPACK_BIRAM(c + 0x730);
    }
}
...
void CMemory::LoROMMap ()
{
	...
	REPACK_BIRAM(c + 0x000);
	REPACK_BIRAM(c + 0x001);
	REPACK_BIRAM(c + 0x800);
	REPACK_BIRAM(c + 0x801);
...
void CMemory::HiROMMap ()
{
	...
	REPACK_BIRAM(0x306 + (c << 4));
	REPACK_BIRAM(0x307 + (c << 4));
	REPACK_BIRAM(0xb06 + (c << 4));
	REPACK_BIRAM(0xb07 + (c << 4));
    }
...
void CMemory::SuperFXROMMap ()
{
	...
	REPACK_BIRAM(0x006 + (c << 4));
	REPACK_BIRAM(0x007 + (c << 4));
	REPACK_BIRAM(0x806 + (c << 4));
	REPACK_BIRAM(0x807 + (c << 4));
    }
...
void CMemory::SRAM1024KLoROMMap ()
{
	...
	REPACK_BIRAM(c + 0x800);
	REPACK_BIRAM(c + 0x801);
	REPACK_BIRAM(c + 0x400);
	REPACK_BIRAM(c + 0x401);
	REPACK_BIRAM(c + 0xc00);
	REPACK_BIRAM(c + 0xc01);
...
void CMemory::BSHiROMMap ()
{
	...
	Map [c + 6] = Map [c + 0x806] = (uint8 *) MAP_NONE;
	Map [c + 7] = Map [c + 0x807] = (uint8 *) MAP_NONE;
	REPACK_BIRAM(c + 0x005);
	REPACK_BIRAM(c + 0x805);
...
void CMemory::ApplyROMFixes ()
{
	...
	for (int c = 0; c < 0x400; c += 16)
	{
		...
		REPACK_BIRAM(c + 0x006);
		REPACK_BIRAM(c + 0x007);
		REPACK_BIRAM(c + 0x806);
		REPACK_BIRAM(c + 0x807);
	}
	WriteProtectROM ();
    }
